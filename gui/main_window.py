...
class FullPipelineThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_message.emit("  - Dang calibrate IPM...")
            ipm = IPMTransformer(cfg)
            ipm.calibrate()

            test_frame = cv2.imread(self.video_path) if isinstance(self.video_path, str) else None
            ...
            if test_frame is not None:
                # Ve 4 diem calibration len anh goc
                calib_frame = ipm.draw_calibration_points(test_frame.copy())
                cv2.imwrite("outputs/calib_check.jpg", calib_frame)
                
                # Tao BEV
                bev_img = ipm.get_bev_image(test_frame)
                cv2.imwrite("outputs/bev_check.jpg", bev_img)
                
                self.log_message.emit("Da luu anh kiem tra: outputs/calib_check.jpg va outputs/bev_check.jpg")
            ...
            # === DOC FRAME DAU TIEN TRUOC ===
            ret, first_frame = cap.read()
            if not ret:
                self.processing_error.emit("Khong doc duoc frame dau tien")
                return
            
            # Bay gio frame da co, goi update_calibration
            ipm.calibrate()  # hoac ipm.update_calibration(first_frame, force=True)
            ...
            # === RESET VIDEO VE DAU ===
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0)  # Quay lai frame 0            
            ...
            while self._running:
                ...
                ipm.update_calibration(frame)

                frame_count += 1

                # === AUTO CALIBRATE MOI 300 FRAME ===
                ...
                try:
                    ...
                    if tracks:
                        feature_vectors = feature_extractor.update(
                            tracks, world_positions, None, frame_count
                        )
                    ...
                    # 7. Ve ket qua
                    # Ve ket qua (bao loi neu co)                    
                    display_frame = self._draw_all(frame, tracks, risk_results, world_positions, ipm)
                    # 8. Dem alerts
                ...
                # Tinh congestion (mac dinh)
                congestion = {'level': 'UNKNOWN', 'density': 0, 'color': (128, 128, 128), 'area_m2': 0}
                
                # Neu IPM hoat dong, tinh congestion that
                if ipm and ipm.is_calibrated:
                    try:
    ...
    def _draw_all(self, frame, tracks, risk_results, world_positions, ipm=None, poses=None):
        """Ve tat ca ket qua len frame - co BEV that tu IPM."""
        result = frame.copy()
        h, w = result.shape[:2]
        
        # === VE VUNG IPM LEN VIDEO ===
        if ipm and ipm.is_calibrated:
            try:
        ...
        # === TAO BEV IMAGE THAT TU IPM ===
        if ipm and ipm.is_calibrated:
            try:
                # Tao BEV image that (da bao gom ROI crop + warp)
                ...
                for track in tracks:
                    if track.track_id in world_positions:
                        ...
                        # World (met) -> BEV pixel (dung ipm.scale)
                ...
                # Ghep vao goc tren phai
                x_offset = w - new_w - 10
                y_offset = 50
                
                # Kiem tra khong vuot qua frame
                ...
                pass  # BEV fail khong anh huong den phan con lai
...
class MainWindow(QMainWindow):
    ...
    def _setup_toolbar(self):
        ...
        toolbar.addAction(QAction("Mo Camera", self, triggered=lambda: self._open_source(0)))
    ...
    def _collect_report_data(self):
        # Thu thập congestion data từ dashboard
        congestion = {
            'north_speed': 12.5, 'north_count': 15,
            'south_speed': 18.2, 'south_count': 22,
            'east_speed': 6.8,  'east_count': 35,
            'west_speed': 24.1, 'west_count': 10,
        }
        ...
        data = {
            ...
            'detection_stats': {
                ...
                'ram_usage': '--',
                'tong_xe_duy_nhat': self.dashboard.vehicle_card.value_label.text(),
